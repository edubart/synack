...
How to generate spoof ips list
  # on target machine
  tcpdump -i eth1 tcp port 9999 -n -t -c 3000000 > spoofsniff
  cat spoof_sniff | awk '{print $2}' | sed 's/^\(.*\)\..*$/\1/' | sort | uniq > spoofips

  # on source machine
  synack -i eth1 -s random -h x.x.x.x -M -t 300 -p 9999 -m 100 -j 10000

How to speed up throughtput got get more pps (packets per second)
  # enable XPS
  ...
  # patch kernel with rawsendto to enable option -k for even more throughtput
  # in the menuconfig make sure you do the following:
  # * disable optimize for size
  # * select you cpu architeture
  # * enable preemption,
  # * change timer clock to 1000Hz,
